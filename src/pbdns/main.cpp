...
int core_loop_unpriv(boost::asio::io_service &io_service,size_t interfaceno,boost::asio::ip::udp::socket &serversocket,std::string yesip) {
  ...
  if (unpriv()) { //Dropping priviledges succeeded. Should always succees unless we are not root to start with.
    if (unpriv.is_child()) {
        ...
        MainServer server(io_service,interfaceno,serversocket,"/etc/pbrouting.json",responsehelper); //Create and register our main server.
        try {
...
int main (int argc, char ** argv) {
  ...
  try {
    //The first argument is the index of our 'client' interface in the config file.
    size_t interfaceno=boost::lexical_cast<size_t>(argv[1]);
    //The second argument is our listen IP. We could have gotten that out of the config ourself, but we need it before dropping priviledges so we don't.
